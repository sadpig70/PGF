...
> When dispatching agents, pass a **PG specification** instead of a natural language prompt.
> A common language for when AI delegates tasks to other AI.
...
|---|---|
| Intent can be ambiguous | I/O is explicit via `def` signature |
| Validation criteria are implicit | `acceptance_criteria` built-in |
| Execution order is buried in prose | Structured with `@dep:`, `→` |
| Failure handling is unclear | Failure Strategy is explicit |
| Result format is inconsistent | Return contract via `-> ReturnType` |

Benefits of communicating via PG:
...
Write the task to be passed to the agent using the following PG structure:
...
| `# failure_strategy:` | Failure response | ○ |
...
When the PGF execute phase encounters a `[parallel]` block:
...
def orchestrate_parallel_block(nodes: list[GantreeNode], design: Path):
    ...
    for node in nodes:
        # 1. Extract TaskSpec from the node's PPR def or # comments
        task_spec = extract_task_spec(node, design)

        # 2. Pass PG-formatted TaskSpec as agent prompt
...
def format_pg_task_spec(spec: TaskSpec) -> str:
    """Convert PG TaskSpec into a prompt the agent can understand

    Key: Convert into executable instructions while preserving PG structure.
    Minimize natural language explanations; the PG specification is the body of the instruction.
    """
...
def implement_discord_adapter(
    ...
    # context:
    #   - Read(channels_crate / "src/adapter.rs")      # verify trait types
...
3. **When executing directly** (due to small scope): TaskSpec not needed, interpret/execute PPR def directly
...
def integrate_agent_results(results: list[TaskResult]) -> IntegrationResult:
    ...
    # 1. Verify all agents met acceptance_criteria
