...
**PGF's ultimate mode.** Autonomously performs the entire discover → design → execute → verify cycle without user approval.
...
def creation_cycle(project_root: str, policy: CreationPolicy) -> CreationResult:
    """PGF autonomous creation cycle — uninterrupted automation of discover → design → execute → verify
...
```text
[PGF CREATE] ✓ Phase 1/5 DISCOVER complete | idea: "{idea_name}" | consensus: CONVERGED
[PGF CREATE] ✓ Phase 2/5 DESIGN complete | nodes: 15 | DESIGN-{Name}.md
[PGF CREATE] ✓ Phase 3/5 PLAN complete | WORKPLAN-{Name}.md + status-{Name}.json
[PGF CREATE] ✓ Phase 4/5 EXECUTE complete | 15/15 nodes done
[PGF CREATE] ✓ Phase 5/5 VERIFY complete | status: passed

[PGF CREATE] ═══ Creation Complete ═══
  Idea: {idea_name}
...
```text
[PGF CREATE] ✗ --skip-discover failed
