...
Discovery Engine is PGF's discovery engine that automatically executes the A3IE 7-stage pipeline using 8 PGF design personas.
...
1. Load `personas.json` (`${Antigravity Agent_SKILL_DIR}/discovery/personas.json`) — metadata including 21 domain list
...
The 8 personas are independently defined as agent files at `${Antigravity Agent_SKILL_DIR}/agents/pgf-persona-p{1~8}.md`.
...
```
Agent(description="A3IE P1 Disruptive Engineer", prompt=task_prompt, agent="${Antigravity Agent_SKILL_DIR}/agents/pgf-persona-p1.md")
Agent(description="A3IE P2 Cold-Eyed Investor", prompt=task_prompt, agent="${Antigravity Agent_SKILL_DIR}/agents/pgf-persona-p2.md")
...
Agent(description="A3IE P8 Convergence Architect", prompt=task_prompt, agent="${Antigravity Agent_SKILL_DIR}/agents/pgf-persona-p8.md")
```
...
```
[PGF CREATE] Phase 1 DISCOVER complete
...
Auto-archive via `${Antigravity Agent_SKILL_DIR}/discovery/archive-discovery.ps1`:

```powershell
# Default invocation
& "$env:Antigravity Agent_SKILL_DIR/discovery/archive-discovery.ps1" -ProjectRoot $projectRoot

# Custom path
& "$env:Antigravity Agent_SKILL_DIR/discovery/archive-discovery.ps1" -DiscoveryDir ".pgf/discovery" -ProjectRoot "D:\MyProject"
```
...
An alternative execution mode leveraging Antigravity's Agent Teams feature for **real-time cross-pollination** between personas.
...
Agent Teams is an experimental Antigravity feature. Fallback to standard subagent mode if Agent Teams is unavailable or encounters errors.
