...
function ConvertTo-EnglishStatus([string]$s) {
    ...
    $ko_done = "$([char]0xC644)$([char]0xB8CC)"           # "done" in Korean
    $ko_prog = "$([char]0xC9C4)$([char]0xD589)$([char]0xC911)" # "in progress" in Korean
    $ko_pend = "$([char]0xC124)$([char]0xACC4)$([char]0xC911)" # "designing" in Korean
    $ko_block = "$([char]0xBCF4)$([char]0xB958)"           # "on hold" in Korean
