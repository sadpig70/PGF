...
- [ ] `AI_make_` causative pattern used where needed (when AI makes a target do something)
- [ ] Considered using micro mode for tasks with ≤10 nodes
- [ ] PG TaskSpec format used when dispatching parallel agents
- [ ] AuthorityBounds (can_create/can_modify/forbidden) specified when delegating
- [ ] Delegation chain depth ≤ 3 confirmed
- [ ] SessionOutcome recorded upon session completion
- [ ] `(delegated)`/`(awaiting-return)`/`(returned)` statuses used appropriately
