...
> **Basic Gantree syntax** is defined in the **PG skill**:
> Node syntax (`NodeName // desc (status) [@dep:]`), 6 basic status codes,
> indentation rules (4 spaces), `[parallel]`, `@dep:`, atomic node judgment (15-minute rule).
>
> This document covers only the **design extension rules** that PGF adds on top of PG.

---

## 1. PGF Additional Status Codes (v2.2)

PGF adds 3 status codes on top of PG's 6 basic statuses (`done`, `in-progress`, `designing`, `blocked`, `decomposed`, `needs-verify`):

| Status | Meaning | AI Execution Rule |
|---|---|---|
| `(delegated)` | Handoff to another agent complete | Skip (running on remote agent) |
...
| Condition | Decision |
|---|---|
| Depth 6+ | Strongly recommend splitting |
...
| Brief PPR | 3-7 line `#` comments below node | Small scale |
