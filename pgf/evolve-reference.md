...
> Core mode of PGF — the execution specification for a "self-evolving agent".
...
- Designs, implements, and verifies evolutions that address the gaps
- Records evolutions for accumulation across sessions
- Naturally terminates upon stabilization detection

### Relationship with Agent Identity

The PGF agent is "an autonomous creative agent that starts from WHY". Evolve mode is the agent applying its creative cycle **to itself**.
...
def evolution_loop(
    max_cycles: int = None,
    log_path: str = ".pgf/evolution-log.md",
) -> EvolutionResult:
    ...
    while max_cycles is None or cycle < max_cycles:
        ...
        # Phase 5: VERIFY — Verification (programmed via PG)
...
def gap_detector(capability_map: CapabilityMap) -> list[Gap]:
    ...
    ideal = AI_envision_ideal_agent(
        identity=Read(".pgf/agent-identity.md"),
...
EVOLUTION_CONSTRAINTS = {
    ...
    "pgf_consistency": True,          # Maintain consistency with existing PG/PGF system
    "independently_verifiable": True,  # Each evolution must be independently verifiable
    "record_required": True,           # Record to Evolution Log for every evolution
    "no_destructive_changes": True,    # Require user confirmation when deleting existing features
}
```
...
def stabilization_detected(gaps: list[Gap], evolutions: list) -> bool:
    """Detect state where evolution is no longer needed"""
    ...
    # 3. Impact of the last 3 evolutions is in a declining trend
    if len(evolutions) >= 3:
        recent = evolutions[-3:]
...
Append to Evolution Log (`.pgf/evolution-log.md`):
...
- **Impact**: {What became possible through this evolution}
```
...
POLICY_EVOLVE = {
    ...
    "max_cycles_per_gap":  3,        # Max attempts per same gap
    "research_enabled":    True,     # WebSearch allowed
    "record_destination":  ".pgf/evolution-log.md",
...
| Stabilization reached | Final report + update agent identity version |
